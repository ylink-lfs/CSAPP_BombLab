...
0000000000400da0 <main>:			;-> Analysis starts from main() function
  400da0:       53                      push   %rbx
  ...
  400e3a:       e8 a1 00 00 00          callq  400ee0 <phase_1>			;-> Use GDB to set breakpoints at every phase_x function for further analysis
...
0000000000400ee0 <phase_1>:
  ...
  400ee4:       be 00 24 40 00          mov    $0x402400,%esi			;-> Use GDB's command "x/50s 0x402400" to check address 0x402400 to find the target string. Then the phase is done.
...
0000000000400efc <phase_2>:
  ...
  400f05:       e8 52 05 00 00          callq  40145c <read_six_numbers>			;-> Step in to check the actual limitation
  400f0a:       83 3c 24 01             cmpl   $0x1,(%rsp)			;-> Check if the first input integer equals to 1
  400f0e:       74 20                   je     400f30 <phase_2+0x34>
  400f10:       e8 25 05 00 00          callq  40143a <explode_bomb>
  400f15:       eb 19                   jmp    400f30 <phase_2+0x34>
  400f17:       8b 43 fc                mov    -0x4(%rbx),%eax
  400f1a:       01 c0                   add    %eax,%eax
  400f1c:       39 03                   cmp    %eax,(%rbx)			;-> The above instructions are a loop to check whether rest numbers are the power of 2, exp, 1, 2, 4, etc.
...
0000000000400f43 <phase_3>:
  ...
  400f60:       83 f8 01                cmp    $0x1,%eax			;-> The input integers terms should be more than 1 term.
  400f63:       7f 05                   jg     400f6a <phase_3+0x27>
  400f65:       e8 d0 04 00 00          callq  40143a <explode_bomb>
  400f6a:       83 7c 24 08 07          cmpl   $0x7,0x8(%rsp)			;-> The first integer should be fewer than 7. I choose 1 for input here.
  400f6f:       77 3c                   ja     400fad <phase_3+0x6a>
  400f71:       8b 44 24 08             mov    0x8(%rsp),%eax
  400f75:       ff 24 c5 70 24 40 00    jmpq   *0x402470(,%rax,8)			;-> Use GDB's step trace to handle the jump details
  ...
  400fbe:       3b 44 24 0c             cmp    0xc(%rsp),%eax			;-> So that 0x137 is the second integer we need.
...
000000000040100c <phase_4>:
  ...
  40102e:       83 7c 24 08 0e          cmpl   $0xe,0x8(%rsp)			;-> The first input value stores at (%rsp + 8)
  401033:       76 05                   jbe    40103a <phase_4+0x2e>
  401035:       e8 00 04 00 00          callq  40143a <explode_bomb>
  40103a:       ba 0e 00 00 00          mov    $0xe,%edx
  40103f:       be 00 00 00 00          mov    $0x0,%esi
  401044:       8b 7c 24 08             mov    0x8(%rsp),%edi
  401048:       e8 81 ff ff ff          callq  400fce <func4>			;-> Plan to use brute method to find the first input value (Ranges from 0-14)
  40104d:       85 c0                   test   %eax,%eax			;-> The return value of func4() should be 0. This is the breakpoint position for brute search.
  40104f:       75 07                   jne    401058 <phase_4+0x4c>			;-> Nice luck...The first trial with "0 0" is successful.
  401051:       83 7c 24 0c 00          cmpl   $0x0,0xc(%rsp)			;-> The second input value stores at (%rsp + 0xe)
...
0000000000401062 <phase_5>:
  ...
  40107f:       83 f8 06                cmp    $0x6,%eax			;-> The input string's length should be 6.
  401082:       74 4e                   je     4010d2 <phase_5+0x70>
  401084:       e8 b1 03 00 00          callq  40143a <explode_bomb>
  401089:       eb 47                   jmp    4010d2 <phase_5+0x70>
  40108b:       0f b6 0c 03             movzbl (%rbx,%rax,1),%ecx			;-> Loop body begins, %ecx stores the current character
  40108f:       88 0c 24                mov    %cl,(%rsp)
  401092:       48 8b 14 24             mov    (%rsp),%rdx			;-> Pick out the current character
  401096:       83 e2 0f                and    $0xf,%edx			;-> Get the lower 4 bit of the character
  401099:       0f b6 92 b0 24 40 00    movzbl 0x4024b0(%rdx),%edx			;-> Get the target character from the string starts in address 0x4024b0. and the offset equals to the value that the lower 4 bits of the original character indicates.
  4010a0:       88 54 04 10             mov    %dl,0x10(%rsp,%rax,1)
  4010a4:       48 83 c0 01             add    $0x1,%rax			;-> Equals to "i++"
  4010a8:       48 83 f8 06             cmp    $0x6,%rax			;-> Equals to "i < 6"
  4010ac:       75 dd                   jne    40108b <phase_5+0x29>			;-> Loop continue
  4010ae:       c6 44 24 16 00          movb   $0x0,0x16(%rsp)
  4010b3:       be 5e 24 40 00          mov    $0x40245e,%esi			;-> After "Manipulation", the input string should be equal to the string hidden in 0x40245e. In my case, "flyers".
  4010b8:       48 8d 7c 24 10          lea    0x10(%rsp),%rdi			;-> Set a breakpoint here to check what the "Manipulation" is perceptually
  ...
  4010d2:       b8 00 00 00 00          mov    $0x0,%eax			;-> Equals to "int i = 0"
...
00000000004010f4 <phase_6>:
  ...
  401106:       e8 51 03 00 00          callq  40145c <read_six_numbers>		;-> Input sequence is 6 integer numbers
  ...
  401121:       76 05                   jbe    401128 <phase_6+0x34>			;-> Implies that the first input integer should in the range of 1 and 6 (Include 1 and 6)
  ...
  40114b:       7e e8                   jle    401135 <phase_6+0x41>			;-> The above loop ensures that the other 5 input integer is not the same as the first one.
  40114d:       49 83 c5 04             add    $0x4,%r13
  401151:       eb c1                   jmp    401114 <phase_6+0x20>			;-> The above loop is nested loop. It ensures that the input 6 integers are all ranges from 1 to 6 and are different from each other.
  ...
  40116d:       75 f1                   jne    401160 <phase_6+0x6c>			;-> The above loop transform each integer x into new integer (7 - x).
  40116f:       be 00 00 00 00          mov    $0x0,%esi
  401174:       eb 21                   jmp    401197 <phase_6+0xa3>			;-> jmp that begins a loop
  401176:       48 8b 52 08             mov    0x8(%rdx),%rdx			;-> The guard jmp's location, 0x6032d8 stores value 0x6032e0.
  ...
  401195:       74 14                   je     4011ab <phase_6+0xb7>			;-> After the iteration step over all 6 integers, go to the next loop.
  401197:       8b 0c 34                mov    (%rsp,%rsi,1),%ecx			;-> Beginning jmp's location
  40119a:       83 f9 01                cmp    $0x1,%ecx			;-> Whether the current location stores int 1
  40119d:       7e e4                   jle    401183 <phase_6+0x8f>
  40119f:       b8 01 00 00 00          mov    $0x1,%eax
  4011a4:       ba d0 32 60 00          mov    $0x6032d0,%edx
  4011a9:       eb cb                   jmp    401176 <phase_6+0x82>			;-> The above loop fills specific number singly corresponds to the 1-6 in the adress beginning at (%rsp + 0x20).
  ...
  4011d2:       48 c7 42 08 00 00 00    movq   $0x0,0x8(%rdx)			;-> Static linked list?
  4011d9:       00
...
000000000040145c <read_six_numbers>:
  ...
  40148f:       83 f8 05                cmp    $0x5,%eax			;-> sscanf() has a return value indicating the successfully read content
  401492:       7f 05                   jg     401499 <read_six_numbers+0x3d>			;-> If the input integers are fewer than 5, then boom...
